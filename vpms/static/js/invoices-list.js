(async function () {
  ...
  renderAppShell(user, "invoices-list.html");
  ...
  try {
      ...
      .map(
        (inv) => `
          <tr class="clickable-row" data-id="${inv.id}">
            ...
            <td>${vendorNameById[inv.vendor_id] || inv.vendor_id}</td>
    ...
    body.querySelectorAll("tr.clickable-row").forEach((row) => {
      row.addEventListener("click", () => {
        window.location.href = `/static/pages/invoice-detail.html?id=${row.dataset.id}`;
      });
    });
  } catch (err) {
    bannerError.textContent = friendlyMessage(err);
