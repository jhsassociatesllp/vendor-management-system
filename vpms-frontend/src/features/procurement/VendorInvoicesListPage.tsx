...
export function VendorInvoicesListPage() {
  ...
  const columns = useMemo<ColumnDef<Invoice>[]>(
    () => [
      { header: 'Invoice Number', accessorKey: 'invoice_number' },
      {
        ...
        // fresh every render, since accessorFn's cached getValue() can go stale if pos
  ...
  return (
    <div>
      <h1 className="mb-space-2 font-heading text-size-1 font-bold">My Invoices</h1>
      <DataTable
        columns={columns}
        data={invoices ?? []}
        isLoading={isLoading}
        ...
        onRowClick={(inv) => navigate(ROUTES.vendorInvoiceTrack + `?id=${inv.id}`)}
      />
    </div>
  )
}
